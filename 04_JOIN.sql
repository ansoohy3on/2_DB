...
- 관계형 데이터베이스는 최소한의 데이터를 테이블에 담고 있어서
  ...
  데이터를 읽어 와야 되는 경우가 많다.
  이때, 테이블 간 관계를 맺기 위한 연결고리 역할이 필요한데,
...
-- 기존에 서로 다른 테이블에 데이터를 조회할 경우 아래와 같이 따로 조회함

-- 직원 번호, 직원명, 부서 코드, (EMPLOYEE)
-- 부서명 (DEPARTMENT)을 조회하고자 할 때

-- 직원 번호, 직원명 ,부서 코드는 EMPLOYEE 테이블에 조회 가능
...
-- * ANSI 표준 구문
-- ANSI는 미국 국립 표준 협회를 뜻함, 미국의 산업 표준을 제정하는 민간 단체로
-- 국제 표준화 기구 ISO에 가입되어 있다.
...
-- ** 오라클 구문은 FULL OUTER JOIN을 사용 못 함
