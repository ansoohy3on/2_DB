...
SELECT EMP_ID, EMP_NAME, JOB_NAME, TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/12) 나이,
        TO_CHAR(SALARY*(1+NVL(BONUS,0))*12, 'L999,999,999') 연봉
FROM EMPLOYEE MAIN
LEFT JOIN JOB USING(JOB_CODE)
WHERE (JOB_CODE, SUBSTR(EMP_NO, 1, 6)) IN (SELECT JOB_CODE, MAX(SUBSTR(EMP_NO, 1, 6))
